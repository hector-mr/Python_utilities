...
Mass = np.round(([star_mass[0]]),1)
Rate = ([float(os.path.dirname(path)[-22]+'.'+os.path.dirname(path)[-21:-18])])
Wind_C_ab = ([float(os.path.dirname(path)[-11]+'.'+os.path.dirname(path)[-10:-8])])
Sim_age = np.round(([age_ign]),0)
Sim_logTc = np.round(([log_center_T[c_ign]]),2)
Sim_logRhoc = np.round(([log_center_Rho[c_ign]]),2)
Sim_mass = np.round(([mass_ign]),4)
Elapsed_time = np.round(([max_age-age_ign]),0)
Final_age = np.round(([max_age]),0)
Final_mass = np.round(([star_mass[-1]]),4)
Final_mass_cc = np.round(([mass_conv_core[-1]]),4)
Frac_final_mass_cc = np.round(([mass_conv_core[-1]/star_mass[-1]]),4)
Incr_eta = np.round(([eta[-1]-eta_ign]),6)
Incr_eta_sun = np.round(([(eta[-1]-eta_ign)/eta_sun]),2)
th = np.round(([max_eps_nuc_t_heat[-1]]),2)
tdyn = np.round(([tdyn[-1]]),2)
logTiso = np.round(([logTiso]),4)




col1=fits.Column(name='Mass',format='f8',array=Mass)
col2=fits.Column(name='Rate',format='f8',array=Rate)
col3=fits.Column(name='Wind_C_ab',format='f8',array=Wind_C_ab)
col4=fits.Column(name='Sim_age',format='i4',array=Sim_age)
col5=fits.Column(name='Sim_logTc',format='f8',array=Sim_logTc)
col6=fits.Column(name='Sim_logRhoc',format='f8',array=Sim_logRhoc)
col7=fits.Column(name='Sim_mass',format='f8',array=Sim_mass)
col8=fits.Column(name='Elapsed_time',format='i4',array=Elapsed_time)
col9=fits.Column(name='Final_age',format='i4',array=Final_age)
col10=fits.Column(name='Final_mass',format='f8',array=Final_mass)
col11=fits.Column(name='Final_mass_cc',format='f8',array=Final_mass_cc)
col12=fits.Column(name='Frac_final_mass_cc',format='f8',array=Frac_final_mass_cc)
col13=fits.Column(name='Incr_eta',format='f8',array=Incr_eta)
col14=fits.Column(name='Incr_eta_sun',format='f8',array=Incr_eta_sun)
col15=fits.Column(name='th',format='f8',array=th)
col16=fits.Column(name='tdyn',format='f8',array=tdyn)
col17=fits.Column(name='logTiso',format='f8',array=logTiso)
cols = fits.ColDefs([col1,col2,col3,col4,col5,col6,col7,col8,col9,col10,col11,col12,col13,col14,col15,col16,col17])
tbhdu = fits.BinTableHDU.from_columns(cols)
prihdr = fits.Header()
prihdr['COMMENT']="The units are 'Msun','Msun/yr','','yr','','','Msun','yr','yr','Msun','Msun','','','etasun','s','s','K'"
prihdu = fits.PrimaryHDU(header=prihdr)
thdulist = fits.HDUList([prihdu, tbhdu])
thdulist.writeto("thermo_runaway"+title+".fits",clobber=True) #clobber=True allows to overwrite the existing FITS table
#thdulist.writeto(pathw+"thermo_runaway"+title+".fits",clobber=True) #clobber=True allows to overwrite the existing FITS table


Info=Table.read("thermo_runaway"+title+".fits",format='fits')
#Info=Table.read(pathw+"thermo_runaway"+title+".fits",format='fits')
