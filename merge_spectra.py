...
for file in os.listdir(path_files):
	if ((file.endswith('.fits')) & ('104yr' in file)):
...
for i in range(len(expl_names_early)):
	...
	expl_files_early[i] = Table.read(path_files + expl_names_early[i],format='fits')
...
expl_names=[]
for file in os.listdir(path_files):
	if ((file.endswith('.fits')) and ('104yr' not in file)):
...
for i in range(len(expl_names)):
	expl_files.append(i)
	# Remove first column (Energy), already present in the early file
	expl_files[i] = Table(Table.read(path_files + expl_names[i],format='fits').columns[1:])
...
for i in range(n1):
	...
	hstack([expl_files_early[i], expl_files[i]]).write(path_write + expl_names[i], overwrite=True)